...
in
{
  ...
  command =
    if !vmHostPackages.stdenv.hostPlatform.isDarwin then
      throw "vfkit only works on macOS (Darwin). Current host: ${system}"
    else if vmHostPackages.stdenv.hostPlatform.isAarch64 != pkgs.stdenv.hostPlatform.isAarch64 then
      throw "vfkit requires matching host and guest architectures. Host: ${system}, Guest: ${pkgs.stdenv.hostPlatform.system}"
    else if user != null then
      throw "vfkit does not support changing user"
    else if balloon then
      throw "vfkit does not support memory ballooning"
    else if rosetta.enable && !vmHostPackages.stdenv.hostPlatform.isAarch64 then
      throw "Rosetta requires Apple Silicon (aarch64-darwin). Current host: ${system}"
    else if devices != [ ] then
      throw "vfkit does not support device passthrough"
    else if credentialFiles != { } then
      throw "vfkit does not support credentialFiles"
    else if vsock.cid != null then
      throw "vfkit vsock support not yet implemented in microvm.nix"
    else if storeOnDisk then
      throw "vfkit does not support storeOnDisk. Use virtiofs shares instead (already configured in examples)."
    else
      let
